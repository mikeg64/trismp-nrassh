...
;;;                  /TRUE_COLOR, $
;;;                  /PS_FULL_RANGE
;;;AS 10/13/06 passing len=3 sets the llength of the vectors to be multiplied
;;;by factor len
;;;            passing maxvec=[a,b] sets maximum number of vectors to [a,b]
;;;   
;;;  or
...
PRO plot_3d_vect, arg1, arg2, arg3, $
                  ...
                  DEBUG=debug,top=top, _EXTRA=extra
...
;; Establish coordinates and size of plot
  ...
  if(yrange(0)-yrange(1) eq 0. and xrange(0)-xrange(1) ne 0.) then yrange=yr
  if(xrange(0)-xrange(1) eq 0. and yrange(0)-yrange(1) ne 0.) then xrange=xr
...
;    TVLCT, ps_rgb(*,0), ps_rgb(*,1), ps_rgb(*,2)
    ...
    if(n_elements(top) gt 0) then print, "top in plot_3d", top

;              tvlct,r,g,b,/get
;              print,"color in plot3d", r
;goto, jump0
    wdtvscl, $
        interpolate(vec_z, $
        ...
        XSIZE=sx, YSIZE=sy, $
        BOTTOM=1, $
        ABSOLUT=absolut, top=top ;,_EXTRA=extra;, top=top
;jump0:



;    if(n_elements(zrange) eq 0) then begin 
;    zrange1=minmax(vec_z)
;endif else begin 
;    zrange1=zrange
;endelse
;print , "zrange=",zrange1
;    wdtvscl, $
;        (interpolate(vec_z, $
;                    (findgen(nx_s*ps_enh)+0.5)/ps_enh-0.5, $
;                    (findgen(ny_s*ps_enh)+0.5)/ps_enh-0.5, $
;                    /grid)-zrange1[0])/(zrange1[1]-zrange1[0])*(n_cols-3.), $
;        xpos, ypos, $
;        XSIZE=sx, YSIZE=sy, $
;        BOTTOM=1, $
;        ABSOLUT=absolut
  ...
  if (keyword_set(autoxy)) then begin
   ...
   length=1,color=255*0+255,/nozero;  


;length=3.5, color=235, maxvec=[64,32]
